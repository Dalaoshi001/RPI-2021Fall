...
-- Pre-calculate the average bilboard ranking
	...
	SELECT 
	EXTRACT(YEAR FROM b.chartdate)::INTEGER - EXTRACT(YEAR FROM b.chartdate)::INTEGER % 10 AS decade
	, avg(rank) as avgrank
	FROM 
		bilboard b
		, songsplayed sp
...
BEGIN
	...
	SELECT abs(decadeavgrank - thisavgrank ) INTO ranksimu;
...
-- Main Body
   ...
   FOR myrow IN 
		SELECT 
			...
			, gsscore(s.id) * w1 + ranksimilarity(s.id) * w2 + songsimularity(s.id) * w3 as songscore
		 FROM 
			songs s
			, artists a
			, songsnotplayed snp
